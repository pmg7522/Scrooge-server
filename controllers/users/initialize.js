...
module.exports = async (req, res) => {
    try {
      ...
      if(data){
        const userinfo = await user.findOne({ where: { id: data.id }, raw: true })
        const { username, email, experience, darkmode, redirect, photo } = userinfo;
        ...
        let photoinfo;
          try {
            ...
            return res.status(200).send({ data: { 
              userInfo: { username, email, photo: realphoto, level: 1, experience }, 
