    ...
    if (data) {
        ...
        include: [
          {
            model: money,
            attributes: [
              [sequelize.fn("sum", sequelize.col("cost")), "allCost"],
            ],
          },
        ],
        group: "category.id",
      ...
      for (let i = 0; i < moneyCategory.length; i++) {
        if (moneyCategory[i]["money.allCost"]) {
          ...
          let categoryrest =
            moneyCategory[i].budget - moneyCategory[i]["money.allCost"];
          let allData = {
            categoryId,
            categoryname,
            categorybudget,
      ...
      for (let i = 0; i < moneyCategory.length; i++) {
        if (moneyCategory[i]["money.allCost"]) {
          usedGraph.push([
            moneyCategory[i].categoryname,
            Number(moneyCategory[i]["money.allCost"]),
          ]);
        } else {
